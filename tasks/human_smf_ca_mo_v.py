...
def main(gpath, cell_smf_task_path, output_path, use_haploinsufficient=False, decile=0):
    ...
    ix = ~pd.isna(df['oe_lof_upper_bin']) & (df['oe_lof_upper_bin'] <= decile)
    ...
    assert len(cell_lethal_genes.intersection(viable_genes)) == 0

    gene_sets = [cell_lethal_genes, final_ma_genes, viable_genes]
    rows = []
