...
def main():
    ...
    full_spec = load_spec("cfgs/tgi_nn_model.json")
    ...
    generate_figures("../results/exp_yeast_tgi/figures")

def run_cv_on_spec(model_spec, name, mode='cv', n_workers=20):
    if name == 'mn':
        dataset_path =  "../generated-data/dataset_yeast_tgi_mn.feather"
    else:
        dataset_path =  "../generated-data/dataset_yeast_tgi.feather"
    models.train_and_evaluate.cv(model_spec, 
                                dataset_path, 
                                "../generated-data/splits/dataset_yeast_tgi_dev_test.npz",
                                mode,
                                "../results/exp_yeast_tgi/%s_%s" % (mode, name),
                                n_workers=n_workers,
                                no_train=False,
...
def generate_figures(output_dir):
    ...
    figure_dev_test_bacc.generate_figures(spec, "../results/exp_yeast_tgi", "../results/exp_yeast_tgi/figures/overall_bacc.png")

    for model in spec['models']:
        model['results_path'] = "../results/exp_yeast_tgi/dev_test_%s/results.json" % model['name']
        figure_cm.plot_cm(model['results_path'], model['color'], spec['short_classes'], 
            "../results/exp_yeast_tgi/figures/cm_%s.png" % model['name'])
    
    for i in range(len(spec['classes'])):
        figure_auc_roc_curve.plot_auc_roc_curves(spec, i, '../results/exp_yeast_tgi/figures/curve_auc_roc_dev_test_%s.png' % spec["short_classes"][i])
