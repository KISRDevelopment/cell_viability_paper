...
def create_plot_df(spec, path):
    ...
    for model in spec['models']:
        ...
        dev_results = load_results(os.path.join(path, "cv_" + model['name'], "results.json"))
        test_results = load_results(os.path.join(path, "dev_test_" + model['name'], "results.json"))
...
def plot_df(spec, df, output_path):
    ...
    for i, b in enumerate(bar.patches):
        ...
        if type > 0:
            ...
            ax.text(model_id, bacc / 2, model['title'], rotation=90, ha="center", va="center", fontsize=model.get('fsize', plot_cfg['bar_label_size']), weight='bold')

            a_ix = (df['model'] == model['title']) & (df['type'] == 'cv')
            a_bacc = df[a_ix]['bacc']
            ...
            yoffset = offset_val
            for j in range(model_id+1, n_models):
                b_ix = (df['model'] == spec['models'][j]['title']) & (df['type'] == 'cv')
                b_bacc = df[b_ix]['bacc']
...
if __name__ == "__main__":
    ...
    generate_figures(spec, "../results/exp_yeast_smf", "../results/exp_yeast_smf/overall_bacc.png")
