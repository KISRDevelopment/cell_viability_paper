...
*/
function populate_gi_triplets(rows)
{
    ...
    rows.forEach((row) => {
        ...
        tds[4].innerHTML = row.gene_c_locus_tag;
        tds[5].innerHTML = row.gene_c_common_name;
        tds[6].innerHTML = row.scl;
        tds[7].innerHTML = row.prob_gi.toFixed(2);
        tds[8].innerHTML = (row.reported_gi === 1) ? '<strong>Yes</strong>' : 'No';
        tds[9].innerHTML = "<span class='btn-details'>Details</span>";

        tds[9].onclick = search_result_clicked;
        tds[9].dataset.a_id = row.gene_a_id;
        tds[9].dataset.b_id = row.gene_b_id;
        tds[9].dataset.c_id = row.gene_c_id;
        tds[9].classList.add('clickable');
